...
const BookListAdvanced = ({ onEdit }) => {
    ...
    const loadInitialData = async () => {
        try {
            ...
            const [booksData, authorsData] = await
Promise.all([
                bookService.getAll(),
...
()))
    ...
    const handleDeleteBook = async (bookId) => {
        ...
        try {
            ...
            setBooks(prev => prev.filter(book => book.id !==
bookId));
        } catch (error) {
    ...
    const getAuthorInfo = (authorIri) => {
        ...
        return authors.find(author => String(author.id) ===
String(authorId));
    };
    if (loading) {
        return <LoadingSpinner message="Chargement de la
bibliothèque..." />;
    }
    if (error) {
        return (
            <div className="error-container">
                <p className="error-message">{error}</p>
                 ...
                 <button onClick={loadInitialData} className="retry-btn">🔄 Réessayer </button>
            </div> 
        );
}
    return (
        <div className="book-list-container">
            <div className="book-list-header">
                ...
                />
                <p className="book-count">{filteredBooks.length} livre{filteredBooks.length > 1 ? 's' : ''}{searchTerm && `
